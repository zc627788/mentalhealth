...
console.info('admin-user-appointments started')
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (category && category !== 'all') {
      if (category === 'human') {
        query = query.eq('appointment_type', 'human')
      } else if (category === 'doubao' || category === 'peppy') {
        query = query.eq('appointment_type', 'ai').eq('ai_model', category)
      }
    }
    ...
    return ok({ success: true, data: { appointments: data || [] }, meta })
  } catch (e: any) {
    console.error('admin-user-appointments error', e)
