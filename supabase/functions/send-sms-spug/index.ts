...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { phoneNumber, templateId = Deno.env.get('SPUG_TEMPLATE_ID') || '', codeLength = 6, ttlSeconds = 600, cooldownSeconds = 180 } = await req.json() as SendSMSPayload

    if (!phoneNumber) return new Response(JSON.stringify({ error: '缺少手机号' } as SendSMSResponse), { status: 400, headers: corsHeaders })
    if (!templateId) return new Response(JSON.stringify({ error: '缺少模板ID' } as SendSMSResponse), { status: 400, headers: corsHeaders })
    ...
    if (!phoneRegex.test(phoneNumber)) return new Response(JSON.stringify({ error: '手机号格式不正确' } as SendSMSResponse), { status: 400, headers: corsHeaders })
    ...
    if (recentErr) {
      ...
      return new Response(JSON.stringify({ error: '系统错误，请稍后重试' } as SendSMSResponse), { status: 500, headers: corsHeaders })
    }
    ...
    if (!spugRes.ok) {
      ...
      return new Response(JSON.stringify({ error: '短信通道错误' } as SendSMSResponse), { status: spugRes.status || 500, headers: corsHeaders })
    }

    return new Response(JSON.stringify({ success: true, message: '验证码已发送' } as SendSMSResponse), { status: 200, headers: corsHeaders })

  } catch (e) {
    console.error('send-sms-spug error:', e)
    return new Response(JSON.stringify({ error: '服务器内部错误' } as SendSMSResponse), { status: 500, headers: corsHeaders })
  }
})
